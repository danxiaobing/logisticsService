...
class Transmanage_InquiryDelModel
{
    ...
    public function getInquiryInfo($id){
        $sql = "SELECT gi.`id`,gi.`status`,gi.`type`,gii.`minprice`,gii.`maxprice`,gii.`type`,gii.`created_at` FROM gl_inquiry gi LEFT JOIN gl_inquiry_info gii ON gi.id = gii.pid WHERE gi.gid=".intval($id)." AND gi.`is_del`=0 AND gi.`status`=1 ORDER BY gii.`id` ASC";
